...
prog="ni-5g-lte-wifi-interworking"
...
echo ""		
case "$2" in
  BSTS) echo "Prepare ns-3 as TS and BS for 5G/LTE/WIFI example."
  # start as terminal station (LTE UE and eNB)
  niApiDevMode="\"NIAPI_BSTS\""	   
  ;;
  MBSTS) echo "Prepare ns-3 as TS and BS for 5G/LTE/WIFI example."
  # start as terminal station (LTE UE and eNB)
  niApiDevMode="\"NIAPI_MBSTS\""	   
  ;;
  SBSTS) echo "Prepare ns-3 as TS and BS for 5G/LTE/WIFI example."
  # start as terminal station (LTE UE and eNB)
...
args="--packetSize=$packetSize --numPackets=$numPackets --interval=$interPacketInterval --simTime=$simTime --transmTime=$transmTime  --niApiDevMode=$niApiDevMode --niApiEnableLogging=$niApiEnableLogging --niApiEnableTapBridge=$niApiEnableTapBridge --niApiWifiEnabled=$niApiWifiEnabled --niApiWifiLoopbackEnabled=$niApiWifiLoopbackEnabled --niApiLteEnabled=$niApiLteEnabled --niApiLteLoopbackEnabled=$niApiLteLoopbackEnabled --cientServerConfig=$cientServerConfig --lwaactivate=$lwaactivate --lwipactivate=$lwipactivate --dcactivate=$dcactivate --fdDeviceName=$fdDeviceName --daliDualConnectivityEnabled=$daliDualConnectivityEnabled --dualConnectivityLaunchTime=$dualConnectivityLaunchTime --usePdcpInSequenceDelivery=$usePdcpInSequenceDelivery --niRemoteControlEnable=$niRemoteControlEnable"
