...
#ifndef DALI_EMU_SEPARATED_INSTANCES_EPC_HELPER_H
#define DALI_EMU_SEPARATED_INSTANCES_EPC_HELPER_H
 ...
 * \ingroup lte
 *
 * \brief Create an EPC network using EmuFdNetDevice with the possibility
 * of connect an eNB (SeNB) to the remote EPC of other ns3 instance (MeNB)
 *
 * This Helper will create an EPC network topology comprising of a
 * single node that implements both the SGW and PGW functionality, and
 * an MME node. S1-U, S1-AP, X2-U, X2-C, DCX-U and DCX-C interfaces are
 * realized using EmuFdNetDevice; in particular, one device is used to send
 * all the traffic related to these interfaces.
 * This implementation allows the execution of specific DALI Dual Connectivity Scenario,
 * with MeNB and SeNB executing on separated instances/hosts. More than 2 eNB or 1 UE per eNB
 * would cause an error of execution. The hard-coded of addresses to link Apps
 * should be automated to prevent this problem.
 */
class DaliEmuSeparatedInstancesEpcHelper : public EpcHelper
{
public:
   ...
   * Constructor
   */
  DaliEmuSeparatedInstancesEpcHelper ();

  /**
   * Destructor
   */
  virtual ~DaliEmuSeparatedInstancesEpcHelper ();
...
private:
  ...
  /**
   * Selection of operation mode for DALI LTE Dual Connectivity, identifying which nodes have fdNetDevices installed
   */
  std::string m_niApiDevMode;
  ...
  /**
   * Record of EPC addresses assignment to sockets and APPs configuration in eNBs.
   */
  Ipv4Address m_sgwAddress;
  Ipv4Address m_mmeAddress;
...
#endif // DALI_EMU_SEPARATED_INSTANCES_EPC_HELPER_H
