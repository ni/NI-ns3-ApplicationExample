 ...
 *          Specific implementation to support DALI Dual Connectivity Scenario,
 *           with MeNB, SeNB, mUE and sUE executing on separated instances/hosts
 */
#include <ns3/dali-emu-separated-instances-epc-helper.h>
#include <ns3/log.h>
...
// TODO Consider implementing X2 interface use same FdNetDevice created for S1-AP interface like propoused by Michele Polese
// TODO Consider automate IP addresses setup on EmuFdNetDevices to extend execution with any number of nodes

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("DaliEmuSeparatedInstancesEpcHelper");

NS_OBJECT_ENSURE_REGISTERED (DaliEmuSeparatedInstancesEpcHelper);


DaliEmuSeparatedInstancesEpcHelper::DaliEmuSeparatedInstancesEpcHelper ()
...
TypeId
DaliEmuSeparatedInstancesEpcHelper::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DaliEmuSeparatedInstancesEpcHelper")
    .SetParent<EpcHelper> ()
    .SetGroupName("Lte")
    .AddConstructor<DaliEmuSeparatedInstancesEpcHelper> ()
	.AddAttribute ("niApiDevMode",
		           "Allows to identify if EmuFdNetDevices has to be installed",
				   StringValue (""),
				   MakeStringAccessor (&DaliEmuSeparatedInstancesEpcHelper::m_niApiDevMode),
				   MakeStringChecker ())
    .AddAttribute ("sgwDeviceName",
                   "The name of the device used for the S1-U interface of the SGW",
                   StringValue ("veth0"),
                   MakeStringAccessor (&DaliEmuSeparatedInstancesEpcHelper::m_sgwDeviceName),
                   MakeStringChecker ())
	.AddAttribute ("mmeDeviceName",
			       "The name of the device used for the S1-MME interface of the MME",
				   StringValue ("veth1"),
				   MakeStringAccessor (&DaliEmuSeparatedInstancesEpcHelper::m_mmeDeviceName),
				   MakeStringChecker ())
    .AddAttribute ("enbS1uDeviceName",
                   "The name of the device used for the S1-U interface of the eNB",
                   StringValue ("veth2"),
                   MakeStringAccessor (&DaliEmuSeparatedInstancesEpcHelper::m_enbS1uDeviceName),
                   MakeStringChecker ())
	.AddAttribute ("enbS1MmeDeviceName",
			       "The name of the device used for the S1-MME interface of the eNB",
				   StringValue ("veth3"),
				   MakeStringAccessor (&DaliEmuSeparatedInstancesEpcHelper::m_enbS1MmeDeviceName),
				   MakeStringChecker ())
	.AddAttribute ("enbX2DeviceName",
				   "The name of the device used for the X2 interfaces of the eNB",
				   StringValue ("veth4"),
				   MakeStringAccessor (&DaliEmuSeparatedInstancesEpcHelper::m_enbX2DeviceName),
				   MakeStringChecker ())
	.AddAttribute ("ueDcxDeviceName",
				   "The name of the device used for the DCX interfaces of the UE",
				   StringValue ("veth5"),
				   MakeStringAccessor (&DaliEmuSeparatedInstancesEpcHelper::m_ueDcxDeviceName),
				   MakeStringChecker ())
	.AddAttribute ("IpMask",
				   "IPv4 addresses mask for devices",
				   Ipv4MaskValue ("255.255.255.0"),
				   MakeIpv4MaskAccessor (&DaliEmuSeparatedInstancesEpcHelper::m_ipMask),
				   MakeIpv4MaskChecker ())
...
TypeId
DaliEmuSeparatedInstancesEpcHelper::GetInstanceTypeId () const
{
  return GetTypeId ();
}

void
DaliEmuSeparatedInstancesEpcHelper::DoInitialize ()
{
  NS_LOG_LOGIC (this);
  ...
  m_mmeIpv4AddressHelper.SetBase (m_mmeIpSubnet, m_ipMask, "0.0.0.1");

  if (m_niApiDevMode == "NIAPI_BS" || m_niApiDevMode == "NIAPI_BSTS" || m_niApiDevMode == "NIAPI_MBS" || m_niApiDevMode == "NIAPI_MBSTS") // Only in case of NIAPI_BS or NIAPI_MBS or NIAPI_MBSTS, EmuFdNetDevice is created and configured for MME and SGW
  {
	...
	m_sgwAddress = m_sgwIpIfaces.GetAddress (0);

	m_mmeIpIfaces = m_mmeIpv4AddressHelper.Assign (mmeDevices);
	m_mmeAddress = m_mmeIpIfaces.GetAddress (0);
  }
  else
  {
    m_sgwAddress = m_epcIpv4AddressHelper.NewAddress();
    m_mmeAddress = m_mmeIpv4AddressHelper.NewAddress();
  }

  // we use a /30 subnet which can hold exactly two addresses for X2 interfaces
  m_x2Ipv4AddressHelper.SetBase (m_x2IpSubnet, "255.255.255.252", "0.0.0.1");
  // we use a /30 subnet which can hold exactly two addresses for DCX interfaces
  m_dcxIpv4AddressHelper.SetBase (m_dcxIpSubnet, "255.255.255.252", "0.0.0.1");

  EpcHelper::DoInitialize ();
}

void
DaliEmuSeparatedInstancesEpcHelper::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
DaliEmuSeparatedInstancesEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
  ...
  NS_ASSERT_MSG (cellId > 2, "ORCA - DALI Dual Connectivity Scenario separated eNB includes only 2 eNBs");

  // Control of Ip Addresses for configurations on eNB
  Ipv4Address enbAddress;
  Ipv4Address mme_enbAddress;
  Ipv4Address startAddress;

  if (cellId==1)
	startAddress = "0.0.0.101";
  else if (cellId==2)
	startAddress = "0.0.0.102";
  //else
	//startAddress = "0.0.0.103";

  m_epcIpv4AddressHelper.SetBase (m_epcIpSubnet, m_ipMask, startAddress);
  m_mmeIpv4AddressHelper.SetBase (m_mmeIpSubnet, m_ipMask, startAddress);
  ...
  if (((m_niApiDevMode == "NIAPI_BS" || m_niApiDevMode == "NIAPI_MBS" || m_niApiDevMode == "NIAPI_MBSTS") && cellId==1)
        || ((m_niApiDevMode == "NIAPI_SBS" || m_niApiDevMode == "NIAPI_SBSTS") && cellId==2) || m_niApiDevMode == "NIAPI_BSTS")
	  //Only in case of eNB1 in MASTER, or eNB2 in SECONDARY, EmuFdNetDevice is created and configured to connect with SGW
  {
	...
	enbAddress = enbIpIfaces.GetAddress (0);
  }
  else
  {
	enbAddress = m_epcIpv4AddressHelper.NewAddress();
  }

  //std::cout << "sgwAddress: " << m_sgwAddress << " enbAddress: " << enbAddress <<  std::endl;
  ...
  if (((m_niApiDevMode == "NIAPI_BS" || m_niApiDevMode == "NIAPI_MBS" || m_niApiDevMode == "NIAPI_MBSTS") && cellId==1)
        || ((m_niApiDevMode == "NIAPI_SBS" || m_niApiDevMode == "NIAPI_SBSTS") && cellId==2) || m_niApiDevMode == "NIAPI_BSTS")
  	  //Only in case of eNB1 in MASTER, or eNB2 in SECONDARY, EmuFdNetDevice is created and configured to connect with MME
  {
	...
	NS_LOG_LOGIC ("number of Ipv4 ifaces of the eNB after assigning Ipv4 addr to S1 dev: " << enb->GetObject<Ipv4> ()->GetNInterfaces ());
	mme_enbAddress = enbapIpIfaces.GetAddress (0);
  }
  else
  {
	mme_enbAddress = m_mmeIpv4AddressHelper.NewAddress();
  }

  //std::cout << "mmeAddress: " << m_mmeAddress << " mme_enbAddress: " << mme_enbAddress <<  std::endl;
  ...
  Ptr<EpcEnbApplication> enbApp = CreateObject<EpcEnbApplication> (enbLteSocket, enbS1uSocket, enbAddress, m_sgwAddress, cellId);
  ...
  uint16_t mmeId = 1;
  Ptr<EpcS1apEnb> s1apEnb = CreateObject<EpcS1apEnb> (enbS1apSocket, mme_enbAddress, m_mmeAddress, cellId, mmeId); // only one mme!
  ...
  m_sgwPgwApp->AddEnb (cellId, enbAddress, m_sgwAddress);
}


void
DaliEmuSeparatedInstancesEpcHelper::AddX2Interface (Ptr<Node> enb1, Ptr<Node> enb2)
{
  ...
  if (m_niApiDevMode == "NIAPI_MBS" || m_niApiDevMode == "NIAPI_MBSTS") //// Only in case of NIAPI_MBS or NIAPI_MBSTS, EmuFdNetDevice is created and configured for X2 interface in eNB 1
  {
    ...
    enb1X2Address = enbIpIfaces.GetAddress (0);
    //emu_enb1.EnablePcap ("/tmp/Dali_MeNB_X2Device", enb1X2Dev);
  }
  else
	enb1X2Address = m_x2Ipv4AddressHelper.NewAddress();

  if (m_niApiDevMode == "NIAPI_SBS" || m_niApiDevMode == "NIAPI_SBSTS") //Only in case of NIAPI_SBS or NIAPI_SBSTS, EmuFdNetDevice is created and configured for X2 interface in eNB 2
  {
    ...
    Ptr<NetDevice> enb2X2Dev = enbX2Devices.Get (0);
    ...
    enb2X2Address = enbIpIfaces.GetAddress (0);
    //emu_enb2.EnablePcap ("/tmp/Dali_SeNB_X2Device", enb2X2Dev);
  }
  else
  	enb2X2Address = m_x2Ipv4AddressHelper.NewAddress();
...
void
DaliEmuSeparatedInstancesEpcHelper::AddUe (Ptr<NetDevice> ueDevice, uint64_t imsi)
{
...
void
DaliEmuSeparatedInstancesEpcHelper::AddDaliUe (Ptr<Node> ue, Ptr<NetDevice> ueDevice, uint64_t imsi)
{
  NS_LOG_FUNCTION (this << imsi << ueDevice );
...
DaliEmuSeparatedInstancesEpcHelper::AddDcxInterface (Ptr<Node> ue1, Ptr<Node> ue2)
{
  ...
  Ipv4Address ue1DcxAddress;
  Ipv4Address ue2DcxAddress;

  if (m_niApiDevMode == "NIAPI_MTS" || m_niApiDevMode == "NIAPI_MBSTS") //// Only in case of NIAPI_MTS or NIAPI_MBSTS, EmuFdNetDevice is created and configured for DCX interface in UE 1
  {
	...
	ue1DcxAddress = ueIpIfaces.GetAddress (0);
	//emu_ue1.EnablePcap ("/tmp/Dali_mUE_DcxDevice", ue1DcxDev);
  }
  else
	ue1DcxAddress = m_dcxIpv4AddressHelper.NewAddress();

  if (m_niApiDevMode == "NIAPI_STS" || m_niApiDevMode == "NIAPI_SBSTS") //// Only in case of NIAPI_STS or NIAPI_SBSTS, EmuFdNetDevice is created and configured for DCX interface in UE 2
  {
	...
	Ptr<NetDevice> ue2DcxDev = ueDcxDevices.Get (0);
	...
	ue2DcxAddress = ueIpIfaces.GetAddress (0);
	//emu_ue2.EnablePcap ("/tmp/Dali_sUE_DcxDevice", ue2DcxDev);
  }
  else
  	ue2DcxAddress = m_dcxIpv4AddressHelper.NewAddress();
...
uint8_t
DaliEmuSeparatedInstancesEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
...
uint8_t
DaliEmuSeparatedInstancesEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, Ptr<EpcUeNas> ueNas, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
  NS_LOG_FUNCTION (this << ueDevice << imsi);
...
Ptr<Node>
DaliEmuSeparatedInstancesEpcHelper::GetPgwNode ()
{
  return m_sgwPgw;
}
...
DaliEmuSeparatedInstancesEpcHelper::AssignUeIpv4Address (NetDeviceContainer ueDevices)
{
  return m_uePgwAddressHelper.Assign (ueDevices);
}

Ipv4Address
DaliEmuSeparatedInstancesEpcHelper::GetUeDefaultGatewayAddress ()
{
