...
#ifndef DALI_LTE_HELPER_H
#define DALI_LTE_HELPER_H
 ...
 *     * NAS
 *     * Specific UE DCX SAPs configuration for DALI LTE Dual Connectivity
 * - EPC helper
 ...
 * SetEpcHelper().
 */
class DaliLteHelper : public Object
{
public:
  DaliLteHelper (void);
  virtual ~DaliLteHelper (void);
...
/**
   * DALI Dual Connectivity
   * Create an DCX interface between all the UEs in a given set.
   *
   * \param ueNodes the set of UE nodes
   */
  void AddDcxInterface (NodeContainer ueNodes);

  /**
   * DALI Dual Connectivity
   * Create an DCX interface between two UEs.
   *
   * \param ueNode1 one UE of the DCX interface
   * \param ueNode2 the other UE of the DCX interface
   */
  void AddDcxInterface (Ptr<Node> enbNode1, Ptr<Node> enbNode2);
...
private:
  ...
  /**
   * The `isDaliDualConnectivity` flag. Set in true if DALI Dual Connectivity is enabled
   */
  bool m_isDaliDualConnectivity;

  /**
   * The `usePdcpInSequenceDelivery` flag. Set in true if DALI PDCP In-sequence delivery of packets is going to be used
   */
  bool m_usePdcpInSequenceDelivery;

  /**
   * The `niApiDevMode` name. Allows to identify the node where NI API has to be installed
   */
  std::string m_niApiDevMode;

  /**
   * The `daliLinkDirection` value. Allows to identify the Link Direction for Evaluation ( DOWNLINK / UPLINK ), therefore activate split bearer in the correspondent node
   */
  std::string m_daliLinkDirection;

}; // end of `class DaliLteHelper`
