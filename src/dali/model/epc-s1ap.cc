...
EpcS1apEnb::DoSendInitialUeMessage (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, uint64_t stmsi, uint16_t ecgi) 
{
  ...
  int bytes = sourceSocket->SendTo (packet, 0, InetSocketAddress (mmeIpAddr, m_s1apUdpPort));
  NI_LOG_CONSOLE_DEBUG("LTE.EPC.S1AP.ENB: Sending "<< bytes << " bytes as InitialUeMessage to socket(" << m_mmeId << "), mmeIpAddr: "
                       << mmeIpAddr);
